...
client.once('ready', async () => {
  ...
  if (!loggedInUser) {
    ...
    if (temAtualizacao) {
      ...
      console.log('     Digite "sim" para atualizar o arquivo ou qualquer outra tecla para continuar.');
    ...
    await printAnimado('     [=] Bem-vindo ao Kurome!');
...
const showMenu = () => {
  ...
  rl.question('     [-] Escolha de acordo:  ', (choice) => {
    switch (choice) {
      ...
      case 'sim': atualizarArquivo(); break;
...
const setStatus = async (client: any, state: string) => {
  ...
  try {
    ...
    const getExtendURL = await RichPresence.getExternal(
      client,
      '1367598902827810906',
      'https://cdn.discordapp.com/icons/1365942465995079711/7fa1e14fe50507ea928f1f3e1df0d26e.webp?size=1024',
      `https://api.victims.lol/api/avatar/${client.user.id}`,
    );

    const status = new RichPresence(client)
      .setApplicationId('1367598902827810906')
      .setType('PLAYING')
      .setURL('https://discord.gg/rjP8QTjZ')
      .setState(state)
      .setDetails(`Versão ${version}`)
      .setName('kurome - cl')
      ...
      .addButton('kurome', 'https://discord.gg/rjP8QTjZ');
